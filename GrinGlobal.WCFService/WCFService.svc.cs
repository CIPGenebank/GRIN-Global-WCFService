...
namespace GrinGlobal.WCFService
{
    ...
    {
        public Stream getdata(string dataview, string parameters)
        {
            ...
            try
            {
                ...
                {
                    var dsResponse = sd.GetData(dataview, parameters, 0, 0, "");
            ...
            {
                JSONString = JsonConvert.SerializeObject(e);
            }
            ...
            try
            {
                ...
                using (SecureData sd = new SecureData(false, httpToken))
                {
                    ...
                    if (ds.Tables["SearchResult"].Rows.Count > 0)
                    {
                        ...
                        {
                            var response = sd.GetData("table_" + tablename, ":" + tablename.Replace("_", "") + "id=" + string.Join(",", ids.ToArray()), 0, 0, "");
                            JSONString = JsonConvert.SerializeObject(response.Tables["table_" + tablename]);
                        }
                        else
                        {
                            var response = sd.GetData(dataview, ":" + tablename.Replace("_", "") + "id= " + string.Join(",", ids.ToArray()), 0, 0, "");
